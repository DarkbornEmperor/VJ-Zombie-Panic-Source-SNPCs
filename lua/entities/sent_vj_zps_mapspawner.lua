...
function ENT:Initialize()
    ...
    self.ZPS_HordeChance = GetConVar("VJ_ZPS_MapSpawner_HordeChance"):GetInt()
    self.ZPS_HordeCooldownMin = GetConVar("VJ_ZPS_MapSpawner_HordeCooldownMin"):GetInt()
    self.ZPS_HordeCooldownMax = GetConVar("VJ_ZPS_MapSpawner_HordeCooldownMax"):GetInt()
    self.ZPS_MaxZombie = GetConVar("VJ_ZPS_MapSpawner_MaxZom"):GetInt()
    ...
    for _, v in ipairs(player.GetAll()) do
        if GetConVar("VJ_ZPS_MapSpawner_Ambient") == 1 then
...
function ENT:FindEnemy()
    ...
    for _, v in pairs(ents.GetAll()) do
        if v.VJ_ID_Living && v:Health() > 0 && !v:IsFlagSet(65536) && (v.VJ_NPC_Class && !VJ.HasValue(v.VJ_NPC_Class, "CLASS_ZOMBIE") or true) then
            table_insert(tbl, v)
        end
    end
    return tbl
...
function ENT:CheckEnemyDistance(ent, remove)
    ...
    if closestDist >= GetConVar("VJ_ZPS_MapSpawner_SpawnMax"):GetInt() + 1000 && !visible && !remove then
