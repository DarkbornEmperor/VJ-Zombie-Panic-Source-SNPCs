...
-- Client Settings ---------------------------------------------------------------------------------------------------------------------------------------------
if CLIENT then
    VJ.AddKillIcon("weapon_vj_zps_grenade", SWEP.PrintName, "zps/killicons/destroy_survivor", Color(0, 160, 255))
end
-- Main Settings ---------------------------------------------------------------------------------------------------------------------------------------------
...
SWEP.NPC_ReloadSound =
    "darkborn/zps/weapons/explosives/grenade/nade_draw.wav"
-- Primary Fire ---------------------------------------------------------------------------------------------------------------------------------------------
...
SWEP.Primary.Sound =
    "darkborn/zps/weapons/explosives/grenade/nade_throw.wav"
...
function SWEP:OnPrimaryAttack(status, statusData)
    if status == "Init" then
        ...
        if IsValid(phys) then
            phys:SetVelocity(self:GetOwner():CalculateProjectile("Curve", self:GetBulletPos(), self:GetOwner():GetEnemy():GetPos() + self:GetOwner():GetEnemy():OBBCenter(), 1200))
        end
    end
